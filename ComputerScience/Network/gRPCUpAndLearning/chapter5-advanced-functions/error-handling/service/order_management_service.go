...
func (s *service) AddOrder(ctx context.Context, req *pb.Order) (*wrapper.StringValue, error) {
	// Example for Error Handling : Throw an error
	if req.Id == "-1" {
		log.Println("Throw an error for AddOrder")

		// 에러 상태 정의
		errorStatus := status.New(codes.InvalidArgument, "Invalid Request received")

		// 구글의 에러 프로토콜 버퍼를 이용하여 에러 상태 설정
		ds, err := errorStatus.WithDetails(&epb.BadRequest_FieldViolation{
			Field:       "ID",
			Description: fmt.Sprintf("Order Id recieved is not valid %s : %s", req.Id, req.Description)})
