...
def min_idx(num_list):
    ...
    length = len(num_list)
    if length == 0:
        print(min_idx.__name__, ': wrong input')
        return
...
def Selection_sort_min(num_list):
    """ - O(n^2) where n is the length of the input list(num_list)
    :param num_list: a list of integers
    ...
    if length==0:
        print(Selection_sort_min.__name__, ': wrong input')
        return
