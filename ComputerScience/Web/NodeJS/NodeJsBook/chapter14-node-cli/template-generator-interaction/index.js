#!/usr/bin/env node

const fs = require('fs');
const path = require('path');
const readline = require('readline');

let rl;
let type = process.argv[2];
let fileName = process.argv[3];
let dstDir = process.argv[4] || '.';


const htmlTemplate = `
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>Template</title>
  </head>
  <body>
    <h1>Template Generator</h1>
    <p>YJ</p>
  </body>
</html>
`;

const routerTemplate = `
const express = require('express');
const router = express.Router();
 
router.get('/', (req, res, next) => {
   try {
     res.send('ok');
   } catch (error) {
     console.error(error);
     next(error);
   }
});
 
module.exports = router;
`;


// 디렉터리 존재 여부 확인 함수
const checkExistence = (dir) => {
  try {
    fs.accessSync(dir, fs.constants.F_OK | fs.constants.R_OK | fs.constants.W_OK);
    return true;
  } catch (err) {
    return false;
  }
};

// 경로 생성 함수
const mkdirp = (dir) => {
  // 상대 경로의 루트부터 하위 디렉터리 
  const dirname = path.relative('.', path.normalize(dir))
    .split(path.sep)
    .filter(p => !!p);

  // 상위 경로부터 순차적으로 디렉터리 생성
  dirname.forEach((d, idx) => {
    // 상위 루트부터 순차적으로 path를 이어서 경로 체크
    const pathBuilder = dirname.slice(0, idx + 1).join(path.sep);

    // 해당 경로가 없으면 생성
    if (!checkExistence(pathBuilder)) {
      fs.mkdirSync(pathBuilder);
    }
  });
};

// main process
const makeTemplate = () => {
  mkdirp(dstDir);
  if (type === 'html') {
    const filePath = path.join(dstDir, `${fileName}.html`);
    if (checkExistence(filePath)) {
      console.error('File already exists');
    } else {
      fs.writeFileSync(filePath, htmlTemplate);
      console.log(filePath, ' is created');
    }
  } else if (type === 'express-router') {
    const filePath = path.join(dstDir, `${fileName}.js`);
    if (checkExistence(filePath)) {
      console.error('File already exists');
    } else {
      fs.writeFileSync(filePath, routerTemplate);
      console.log(filePath, ' is created');
    }
  } else {
    console.error('type must be "html" or "express-router"');
  }
};

// 경로 지정 함수
const pathAnswer = (answer) => {
  dstDir = (answer && answer.trim()) || '.';
  rl.close();

  makeTemplate();
};

// 파일 이름 지정 함수
const fileNameAnswer = (answer) => {
  if (!answer || !answer.trim()) {
    console.clear();
    console.log('Have to enter "name"');
    return rl.question('Enter your file name : ', fileNameAnswer);
  }
  fileName = answer;

  return rl.question('Enter your directory : ', pathAnswer);
};

// 타입 이름 지정 함수
const typeAnswer = (answer) => {
  if (answer !== 'html' && answer !== 'express-router') {
    console.clear();
    console.log('Type must be "html" or "express-router"');

    return rl.question('Enter the template type : ', typeAnswer);
  }

  type = answer;
  return rl.question('Enter your file name : ', fileNameAnswer);
};

// 메인
const main = () => {
  if (!type || !fileName) {
    rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout,
    });

    console.clear();
    return rl.question('Enter the template type : ', typeAnswer);
  } else {
    makeTemplate();
  }
};

main();