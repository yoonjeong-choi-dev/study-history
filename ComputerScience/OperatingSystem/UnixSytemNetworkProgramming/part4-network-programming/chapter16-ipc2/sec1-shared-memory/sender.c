...
int main()
{
    ...
    while(is_run)
    {
        // 첫 2바이트 플래그가 NO일 때까지 대기 
        while(!strncmp(buffer, "ON", 2))
        {
            // 0.01초간 휴식 : cpu 점령 방지용
            usleep(10000);
        }

        // 첫 2바이트 플래그가 NO인 경우, 메시지 입력 후, 첫 2바이트 플래그 ON으로 변경
        printf("Enter your message('quit' to end) : ");
        gets(buffer+2);
        strncpy(buffer, "ON", 2);
    ...
    // 공유 메모리를 프로세스에서 분리
    // 공유 메모리 제거는 receiver 프로세스에서 실행함
