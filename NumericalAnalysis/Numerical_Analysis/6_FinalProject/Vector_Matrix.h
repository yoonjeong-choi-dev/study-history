...
public:
	...
	void print(bool fix_precision = false, bool option = false);
	void release();
	void clear();
...
template<typename T>
void FVector<T>::setAll(T element) {
	for (int i = 0; i < _size; i++)
		_vec[i] = element;
}
...
template<typename T>
void FMatrix<T>::print(bool fix_precision = false, bool option = false) {
	if (option) {
		cout << " FMatrix : " << endl;
	}
